...
def test_ir_fixed_float_swap_ctor():
    ...
    floating_rate_curve_id = InterestRateCurveId(
        InterestRateIndex.LIBOR, currency, cashflow_tenor
    )
    ...
    floating_rate_cashflow = FloatingRateCashflow(
        notional, currency, cashflow_tenor, floating_rate_curve_id
    )

    ir_fixed_leg = IRFixedLeg.build_from_single_cashflow_definition(
        cashflow_dates, fixed_rate_cashflow
    )
    ir_floating_leg = IRFloatingLeg.build_from_single_cashflow_definition(
